...
class EntityScheduleRadOncMigration extends Migration {
  ...
  public function __construct($arguments) {
    ...
    $this->addFieldMapping('field_patient_reference1', 'pat_id1')
      ->description('lookup existing pat-id1 in prepareRow');
    $this->addFieldMapping('field_notes', 'notes');

    // send these schedules to Rosalio, who owns the radonc.  UID to 301!
    $this->addFieldMapping('uid')->defaultValue('301');
...
//        watchdog('schedule_migration', "query matches: $nid");
       ...
       watchdog('schedule_migration', "radonc no patient found? : $row->lastname, $row->firstname, $row->pat_id1");
       return;
      }
    }else{
        //there is no MRN, this is an orphan order
        watchdog('schedule_migration', "orphan radonc schedule : $row->lastname, $row->start_date");
        return;
    }
    return $nid;
  }
